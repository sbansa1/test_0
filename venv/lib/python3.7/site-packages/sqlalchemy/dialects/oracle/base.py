...
* the values passed for the limit/offset are sent as bound parameters.   Some
  user_service have observed that Oracle produces a poor query plan when the values
...
Some user_service have reported better performance when the entirely different
approach of a window query is used, i.e. ROW_NUMBER() OVER (ORDER BY), to
provide LIMIT/OFFSET (note that the majority of user_service don't observe this).
    ...
    def _prepare_reflection_args(
        ...
        **kw
        ...
        if dblink:
            ...
            # will need to hear from more user_service if we are doing
