...
class AbstractIOServices(pika.compat.AbstractBase):
    """Interface to I/O service required by `pika.adapters.BaseConnection` and
    related utilities.

    NOTE: This is not a public API. Pika user_service should rely on the native I/O
    ...
    def close(self):
        ...
        implementations. Pika user_service should use the native I/O loop's API
        instead.
    ...
    def run(self):
        ...
        methods named start/stop). Pika user_service should use the native I/O loop's
        API instead.
...
class AbstractFileDescriptorServices(pika.compat.AbstractBase):
    """Interface definition of common non-blocking file descriptor service
