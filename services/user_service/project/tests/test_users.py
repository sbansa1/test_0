...
from serv.user_service import User
...
def test_single_user(test_app, test_database, add_user):
    ...
    client = test_app.test_client()
    resp = client.get(f"/user_service/{user.id}")
    data = json.loads(resp.data.decode())
...
def test_remove_user(test_app, test_database, add_user):
    ...
    client = test_app.test_client()
    resp_one = client.get("/user_service")
    data = json.loads(resp_one.data.decode())
    assert resp_one.status_code == 200
    assert len(data) == 1
    resp_two = client.delete(f"/user_service/{user.id}")
    data = json.loads(resp_two.data.decode())
    assert resp_two.status_code == 200
    assert "remove-me@testdriven.io was removed!" in data["message"]
    resp_three = client.get("/user_service")
...
def test_update_user(test_app, test_database, add_user):
    ...
    resp_two = client.get(f"/user_service/{user.id}")
    data = json.loads(resp_two.data.decode())
    assert resp_two.status_code == 200
