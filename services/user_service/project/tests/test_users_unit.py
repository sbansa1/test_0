...
def test_add_user(test_app, monkeypatch):
    ...
    monkeypatch.setattr(
        services.users.project.api.users.views, "get_user_by_email", mock_get_user_by_email
    )
    monkeypatch.setattr( services.users.project.api.users.views, "add_user", mock_add_user )
...
def test_add_user_invalid_json(test_app):
    client = test_app.test_client()
    resp = client.post("/user_service", data=json.dumps({}), content_type="application/json",)
    data = json.loads(resp.data.decode())
...
def test_single_user(test_app, monkeypatch):
    ...
    monkeypatch.setattr( services.users.project.api.users.views, "get_user_by_id", mock_get_user_by_id )
    client = test_app.test_client()
    resp = client.get("/user_service/1")
    data = json.loads(resp.data.decode())
...
def test_single_user_incorrect_id(test_app, monkeypatch):
    ...
    monkeypatch.setattr( services.users.project.api.users.views, "get_user_by_id", mock_get_user_by_id )
    client = test_app.test_client()
    resp = client.get("/user_service/999")
    data = json.loads(resp.data.decode())
...
def test_all_users(test_app, monkeypatch):
    ...
    monkeypatch.setattr( services.users.project.api.users.views, "get_all_users", mock_get_all_users )
    client = test_app.test_client()
    resp = client.get("/user_service")
    data = json.loads(resp.data.decode())
    assert resp.status_code == 200
    assert len(data) == 2
...
def test_remove_user(test_app, monkeypatch):
    ...
    monkeypatch.setattr( services.users.project.api.users.views, "get_user_by_id", mock_get_user_by_id )
    monkeypatch.setattr( services.users.project.api.users.views, "delete_user", mock_delete_user )
    client = test_app.test_client()
    resp_two = client.delete("/user_service/1")
...
def test_remove_user_incorrect_id(test_app, monkeypatch):
    ...
    monkeypatch.setattr( services.users.project.api.users.views, "get_user_by_id", mock_get_user_by_id )
    client = test_app.test_client()
    resp = client.delete("/user_service/999")
    data = json.loads(resp.data.decode())
    assert resp.status_code == 404
    assert "User 999 does not exist" in data["message"]
...
def test_update_user(test_app, monkeypatch):
    ...
    monkeypatch.setattr( services.users.project.api.users.views, "get_user_by_id", mock_get_user_by_id )
    monkeypatch.setattr( services.users.project.api.users.views, "update_user", mock_update_user )
    client = test_app.test_client()
    resp_one = client.put(
        "/user_service/1",
    ...
    resp_two = client.get("/user_service/1")
    data = json.loads(resp_two.data.decode())
    assert resp_two.status_code == 200
...
def test_update_user_invalid(
    ...
    client = test_app.test_client()
    resp = client.put(
        f"/user_service/{user_id}", data=json.dumps(payload), content_type="application/json",
    )
    data = json.loads(resp.data.decode())
